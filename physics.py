...
def get_solution(sol, data):
    ...
    for idx, sol_id in enumerate(sol):
        if idx == 0: solution = data["equations"][sol_id]
        else: solution += ", " + data["equations"][sol_id]
    return solution
...
def get_phyQ(topic):
    ...
    with open("LLM_CONFIG/config.json", "r") as file:
        env = json.load(file)
    env_obj_rag = RagAgent(model_name=env["LLM_MODEL"]["INNER_MODEL"], 
                    topic_name=topic.env_name)
    try:
...
if __name__ == '__main__':
    args = argparse.ArgumentParser(description="Argparser for Physics Question Generation.")
    args.add_argument("--trials", type = int, default = 1, help = "Number of physics questions to generate per topic")
